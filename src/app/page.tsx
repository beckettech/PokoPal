'use client'

import { useEffect } from "react";
import { useAppStore } from "@/lib/store";
import { hapticTap } from "@/lib/haptics";
import { setPageBackground } from "@/lib/page-bg";
import dynamic from "next/dynamic";
import { HomePage } from "@/components/pages/home-page";
import { DexPage } from "@/components/pages/dex-page";
import { MapPage } from "@/components/pages/map-page";
import { RequestsPage } from "@/components/pages/requests-page";

const HabitatDexPage = dynamic(() => import("@/components/pages/habitat-dex-page").then(m => ({ default: m.HabitatDexPage })));
const CloudIslandsPage = dynamic(() => import("@/components/pages/cloud-islands-page").then(m => ({ default: m.CloudIslandsPage })));
const DreamIslandsPage = dynamic(() => import("@/components/pages/dream-islands-page").then(m => ({ default: m.DreamIslandsPage })));
const RelicsPage = dynamic(() => import("@/components/pages/relics-page").then(m => ({ default: m.RelicsPage })));
const ItemsPage = dynamic(() => import("@/components/pages/items-page").then(m => ({ default: m.ItemsPage })));
const MysteryGiftsPage = dynamic(() => import("@/components/pages/mystery-gifts-page").then(m => ({ default: m.MysteryGiftsPage })));
const ChatPage = dynamic(() => import("@/components/pages/chat-page").then(m => ({ default: m.ChatPage })));
const CoinShopPage = dynamic(() => import("@/components/pages/coin-shop-page").then(m => ({ default: m.CoinShopPage })));
const AccountPage = dynamic(() => import("@/components/pages/account-page").then(m => ({ default: m.AccountPage })));
import { MobileAdBanner } from "@/components/MobileAdBanner";
import { BroadcastBanner } from "@/components/BroadcastBanner";
import { AdminToolbar } from "@/components/AdminToolbar";

export default function Home() {
  const currentPage = useAppStore((state) => state.currentPage);
  const setCurrentPage = useAppStore((state) => state.setCurrentPage);
  const darkMode = useAppStore((state) => state.darkMode);
  const markPageVisited = useAppStore((state) => state.markPageVisited);
  const user = useAppStore((s) => s.user);
  const adminForceAds = useAppStore((s) => s.adminForceAds);
  const showAds = adminForceAds !== 'hide' && !(adminForceAds === 'default' && (user.isPremium || user.adsRemoved));

  // Global haptic on all button clicks
  useEffect(() => {
    const handler = (e: Event) => {
      const target = e.target as HTMLElement;
      if (target.closest('button') || target.closest('[role="button"]') || target.closest('a')) {
        hapticTap();
      }
    };
    window.addEventListener('click', handler, { passive: true });
    return () => window.removeEventListener('click', handler);
  }, []);

  // Set native background color to match current page
  useEffect(() => {
    setPageBackground(currentPage);
  }, [currentPage]);

  // Configure RevenueCat on native
  useEffect(() => {
    (async () => {
      const { isNativePlatform, configurePurchases } = await import('@/lib/purchases');
      if (await isNativePlatform()) configurePurchases();
    })();
  }, []);

  // Track page visits for notification badges
  if (currentPage !== "home") {
    markPageVisited(currentPage);
  }
  
  const renderPage = () => {
    switch (currentPage) {
      case "home":
        return <HomePage />;
      case "dex":
        return <DexPage />;
      case "habitat-dex":
        return <HabitatDexPage />;
      case "map":
        return <MapPage />;
      case "requests":
        return <RequestsPage />;
      case "cloud-islands":
        return <CloudIslandsPage />;
      case "dream-islands":
        return <DreamIslandsPage />;
      case "relics":
        return <RelicsPage />;
      case "items":
        return <ItemsPage />;
      case "mystery-gifts":
        return <MysteryGiftsPage />;
      case "chat":
        return <ChatPage />;
      case "coin-shop":
        return <CoinShopPage />;
      case "account":
        return <AccountPage />;
      default:
        return <HomePage />;
    }
  };

  return (
    <main className={`h-screen overflow-hidden ${darkMode ? 'dark bg-gray-900' : 'bg-gradient-to-b from-red-500 to-red-700'}`}>
      <div key={currentPage} className={`h-full ${showAds ? 'pb-[80px]' : ''}`}>
        {renderPage()}
      </div>
      <MobileAdBanner />
      {/* Remove Ads CTA — pulsing button below ad banner */}
      {showAds && !user.isPremium && !user.adsRemoved && (
        <button
          onClick={() => setCurrentPage("account")}
          className="fixed bottom-0 left-0 right-0 z-40 py-2 text-center font-bold text-sm tracking-wide text-white animate-pulse"
          style={{
            background: 'linear-gradient(90deg, #f59e0b, #ef4444, #8b5cf6, #06b6d4, #f59e0b)',
            backgroundSize: '200% 100%',
            animation: 'pulse 2s ease-in-out infinite, rainbow 3s linear infinite',
          }}
        >
          🚫 Remove Ads
        </button>
      )}
      <BroadcastBanner />
      <AdminToolbar />
      <style>{`
        @keyframes rainbow {
          0% { background-position: 0% 50%; }
          100% { background-position: 200% 50%; }
        }
      `}</style>
    </main>
  );
}
